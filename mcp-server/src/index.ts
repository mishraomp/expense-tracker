...
// Operations already covered by a curated, hand-written tool above — skipped
// when auto-registering the rest of the API surface from the OpenAPI spec so
// each endpoint gets exactly one tool.
...
export async function createServer(): Promise<McpServer> {
  ...
  try {
    const { registered } = await registerOpenApiTools(server, { skipOperationIds: CURATED_OPERATION_IDS });
    console.error(`[expense-tracker-mcp] Registered ${registered} additional tools from the OpenAPI spec.`);
  } catch (err) {
    console.error(
