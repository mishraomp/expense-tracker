...
type CategoryTagBreakdownItem = {
  categoryName: string;
  categoryColorCode?: string | null;
  tagName?: string | null;
  tagColorCode?: string | null;
...
interface CategoryTagBreakdownChartProps {
  data: CategoryTagBreakdownItem[];
}
...
export const CategoryTagBreakdownChart = ({ data }: CategoryTagBreakdownChartProps) => {
  ...
  useEffect(() => {
    ...
    svg.append('title').attr('id', 'pie-title').text('Spending by Category & Tags');
    svg
      .append('desc')
      .attr('id', 'pie-desc')
      .text(`Pie chart showing spending distribution across ${data.length} category/tag groups`);

    const g = svg.append('g').attr('transform', `translate(${width / 2},${height / 2})`);

    const parsed = data.map((d, i) => {
      const tagName = d.tagName ?? 'Untagged';
      const label = `${d.categoryName} / ${tagName}`;
      const color =
        d.tagColorCode || d.categoryColorCode || DEFAULT_COLORS[i % DEFAULT_COLORS.length];
      return {
        label,
    ...
    const parsedWithPercentage: PieData[] = parsed.map((d) => ({
      ...d,
    ...
    paths
      .append('title')
      .text(
        (d) => `${d.data.label}: $${d.data.amount.toFixed(2)} (${d.data.percentage.toFixed(1)}%)`,
      );
    ...
    const legend = svg.append('g').attr('role', 'list').attr('aria-label', 'Category/tag legend');
    if (width > 600) {
      legend.attr('transform', `translate(${width - 200}, 20)`);
    } else {
