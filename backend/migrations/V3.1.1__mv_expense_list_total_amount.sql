-- V3.1.1__mv_expense_list_total_amount.sql
-- Update mv_expense_list so the `amount` field reflects total amount (base + GST + PST)
-- Previously, `amount` reflected only the base expense amount.
...
SELECT
    ...
    e.category_id,
    e.subcategory_id,
    (e.amount + COALESCE(e.gst_amount, 0) + COALESCE(e.pst_amount, 0)) AS amount,
    e.date,
    e.description,
    e.source,
    e.status,
...
COMMENT ON MATERIALIZED VIEW mv_expense_list IS
    'Denormalized expense list for fast filtering, sorting, and pagination. Amount is total (base + GST + PST). Refreshed on expense CRUD.';
